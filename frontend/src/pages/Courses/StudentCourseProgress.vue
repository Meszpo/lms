					...
					>
						...
						<ul class="list-none">
							<li
								...
								class="flex justify-between text-sm py-2 my-1 gap-x-2"
							>
								<div class="min-w-0">
								...
								<div class="flex items-center gap-x-2 shrink-0">
									<template v-if="getLabForLesson(progress)">
										<Tooltip :text="__('Lab')">
											<span
												class="lucide-flask-conical size-3.5 text-ink-gray-5"
											/>
										</Tooltip>
										<span class="text-xs text-ink-gray-7">
											{{
												Math.ceil(
													getLabForLesson(progress)
														.percentage
												)
											}}%
										</span>
								...
								<Tooltip v-else :text="__('Pending')">
									<span
										class="lucide-minus text-ink-amber-5 size-4"
									/>
								</Tooltip>
							</div>
							</li>
...
const hasAssessmentData = computed(() => {
	return (
		...
		(assessmentProgress.data?.labs &&
			assessmentProgress.data.labs.length > 0)
	)
})
