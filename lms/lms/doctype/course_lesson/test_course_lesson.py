...
class TestApplyEnforcementFlagsEdgeCases(unittest.TestCase):
	...
	def test_dict_subclass_input(self):
		...
		self.assertEqual(self.fn(quiz_done=False, assignment_done=False, settings=settings), (True, False, True))
	...
	def test_string_zero_is_truthy_treated_as_enforced(self):
		...
		self.assertEqual(self.fn(quiz_done=False, assignment_done=False, settings=settings), (False, False, True))

	def test_string_one_treated_as_enforced(self):
		settings = {"enforce_quiz_completion": "1", "enforce_assignment_completion": "1"}
		self.assertEqual(self.fn(quiz_done=True, assignment_done=True, settings=settings), (True, True, True))
		self.assertEqual(self.fn(quiz_done=False, assignment_done=True, settings=settings), (False, True, True))
