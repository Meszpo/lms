...
def cleanup_lesson_backreferences(lesson: str):
	...
	if frappe.db.exists("DocType", "LMS Lab Submission"):
		for submission in frappe.get_all("LMS Lab Submission", {"lesson": lesson}, pluck="name"):
			frappe.delete_doc("LMS Lab Submission", submission, ignore_permissions=True)
...
def apply_enforcement_flags(
	quiz_done: bool, assignment_done: bool, settings: dict, lab_done: bool = True
...
def save_progress(lesson: str, course: str, scorm_details: dict = None, member: str = None):
	...
	with batched_enrollment_updates():
		return _save_progress(lesson, course, scorm_details, member=member)


def _save_progress(lesson: str, course: str, scorm_details: dict = None, member: str = None):
	member = member or frappe.session.user
	...
	try:
		settings = (
			frappe.get_cached_value(
				"LMS Settings",
				None,
				["enforce_quiz_completion", "enforce_assignment_completion", "enforce_lab_completion"],
				as_dict=True,
			)
			or {}
		)
